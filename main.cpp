...
int main(){
    ...
    while (cin >> linha >> coluna) {
        ...
        for (i = 0; i < partida.get_jogadas_possiveis_linha().size() && i < partida.get_jogadas_possiveis_coluna().size(); ++i) {
        ...
        if (i == partida.get_jogadas_possiveis_linha().size() || i == partida.get_jogadas_possiveis_coluna().size()) {
            cout << "ERRO: jogada inválida" << endl;
            continue;
        }
